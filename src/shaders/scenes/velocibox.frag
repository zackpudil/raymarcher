...
float dObstacles(vec3 p, float diff) {
    ...
    if(time >= 0.0 && time <= 4.0) {
        if(time <= 3.0) o = 4.0;
        else o = mix(4.0, 0.0, mod(time, 1.0));
    } else o = 0.0;
...
vec3 boxPos(vec3 p, float diff, float o) {
    ...
    bp.y = 1.8*cos(sin(time + o)*3. + time + o);
...
vec2 map(vec3 p) {
    ...
    vec2 s = vec2(min(len(p - bp2, 1.0) - .25, dSegment(p, bp, bp2, .04)), 1.0);
...
mat3 camera(vec3 e, vec3 l) {
    vec3 rl = vec3(sin(time), cos(time), 0);
